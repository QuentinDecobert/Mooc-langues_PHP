...
<div id="container" class="quiz with-menu">
    <div class="row">
        ...
        if (! $session->get('last') ) :
            ...
            ?>
            <h4>Connecté en temps que : <strong><?php echo $user->getName(); ?></strong></h4>
            ...
            <p>
                ...
                <input type="submit" id="submit" class="btn btn-primary" name="submit" value="Valider" />
            </p>
        </form>
        <?php else:
            ...
            ?>
            <div id="finalscore">
            <h2 id="score"><?php echo $user->getName(); ?> a eu <?php echo $session->get('score'); ?> réponse(s) correcte(s) sur <?php echo $quiz->countQuestions(); ?></h2>
            <h2 class="userscore"><?php echo $percentage; ?>%</h2>
            <h3 id="time">Temps: <?php echo $timetaken['mins'] . $timetaken['secs']; ?></h3>

            <p id="compare"><a href="<?php echo $root; ?>/quiz/<?php echo $quiz->getId(); ?>/results">Comparez votre résultat!</a></p>
            </div>
        <?php endif; ?>
        </div>
    </div>
